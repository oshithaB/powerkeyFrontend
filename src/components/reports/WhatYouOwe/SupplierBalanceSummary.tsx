...
const SupplierBalanceSummary: React.FC = () => {
  ...
  const fetchSupplierBalanceData = async (startDate?: string, endDate?: string) => {
    ...
    setError(null);
    try {
      const response = await axiosInstance.get(`https://powerkeybackend-production.up.railway.app/api/supplier-balance-summary/${selectedCompany.company_id}`, {
        params: { start_date: startDate, end_date: endDate }
      });
      console.log(response.data);
      setData(response.data.data);
    } catch (err) {
      setError('Failed to fetch supplier balance data. Please try again.');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (selectedCompany?.company_id) {
      ...
      fetchSupplierBalanceData(startDate, endDate);
    } else {
      setError('Please select a company to view supplier balance summary');
      setLoading(false);
    }
  ...
  const getTotalBalance = () => {
    return data.reduce((total, supplier) => total + parseFloat(supplier.total_balance_due), 0);
  };
  ...
  const handleSupplierClick = (vendorId: string) => {
    navigate(`/reports/supplier-balance-detail/${vendorId}`);
  };
  ...
  const handleDownloadPDF = async () => {
    try {
      if (printRef.current) {
        ...
        const filename = `supplier-balance-summary-${new Date().toISOString().split('T')[0]}.pdf`;
        pdf.save(filename);
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center">
          <p className="text-gray-600">Please select a company to view supplier balance summary.</p>
          <button
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="relative top-4 mx-auto p-5 border w-full max-w-7xl shadow-lg rounded-md bg-white">
            <div className="flex justify-between items-center mb-4">
              <h1 className="text-2xl font-bold mb-4">Supplier Balance Summary</h1>
            ...
            <div id="print-content">
              <div className="flex justify-between items-center mb-4">
                <p className="text-sm">Outstanding Supplier Balances - {selectedCompany.name}</p>
              ...
              {!loading && !error && data.length === 0 && (
                <div className="text-center py-8 text-gray-500">
                  No supplier balance data available for the selected period.
                </div>
              )}
              ...
              {!loading && !error && data.length > 0 && (
                <table className="w-full border-collapse">
                  <thead>
                    <tr>
                      <th className="bg-gray-100 p-2 font-semibold text-lg border-b section-header text-left" style={{backgroundColor: '#e2e8f0', WebkitPrintColorAdjust: 'exact', colorAdjust: 'exact', printColorAdjust: 'exact'}}>Supplier Name</th>
                  ...
                  <tbody>
                    {data.map((supplier, index) => (
                      <tr 
                        ...
                        onClick={() => handleSupplierClick(supplier.vendor_id)}
                        className="hover:bg-gray-50 cursor-pointer transition-colors"
                      >
                        <td className="p-2 border-b">{supplier.customer_name}</td>
                        <td className="p-2 border-b">{supplier.email || 'N/A'}</td>
                        <td className="p-2 border-b">{supplier.phone || 'N/A'}</td>
                        <td className="p-2 border-b text-right">{formatCurrency(supplier.total_balance_due)}</td>
                      </tr>
                    ))}
                    <tr>
                      <td className="p-2 border-t-2 border-gray-800 font-bold" colSpan={3}>Total Outstanding Balance</td>
                      <td className="p-2 border-t-2 border-gray-800 font-bold text-right">{formatCurrency(getTotalBalance().toString())}</td>
                    </tr>
                  </tbody>
                </table>
              )}
          ...
          <div className="relative top-4 mx-auto p-5 border w-full max-w-5xl shadow-lg rounded-md bg-white">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-medium text-gray-900">
                Print Preview - Supplier Balance Summary
              </h3>
              <button
            ...
            <div className="overflow-y-auto max-h-[70vh]">
              ...
              >
                ...
                <div className="flex justify-between items-start border-b pb-4 mb-6">
                  <div>
                    <h1 className="text-3xl font-bold mb-2">Supplier Balance Summary</h1>
                    <h2 className="text-xl text-gray-600 mb-2">Outstanding Supplier Balances</h2>
                ...
                <table className="w-full border-collapse mb-6">
                  <thead>
                    <tr>
                      <th className="bg-gray-100 p-2 font-bold text-base border section-header text-left" style={{backgroundColor: '#e2e8f0', WebkitPrintColorAdjust: 'exact', colorAdjust: 'exact', printColorAdjust: 'exact'}}>SUPPLIER NAME</th>
                  ...
                  <tbody>
                    {data.map((supplier, index) => (
                      <tr key={supplier.vendor_id}>
                        <td className="p-2 border-b">{supplier.customer_name}</td>
                        <td className="p-2 border-b">{supplier.email || 'N/A'}</td>
                        <td className="p-2 border-b">{supplier.phone || 'N/A'}</td>
                        <td className="p-2 border-b text-right font-medium">{formatCurrency(supplier.total_balance_due)}</td>
                      </tr>
                    ))}
                    <tr>
                      <td className="p-2 border-t-2 border-gray-800 font-bold" colSpan={3}>TOTAL OUTSTANDING BALANCE</td>
...
export default SupplierBalanceSummary;