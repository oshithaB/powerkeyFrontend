...
const CustomerBalanceSummary: React.FC = () => {
  ...
  const [data, setData] = useState<CustomerBalanceData[]>([]);
  ...
  const fetchCustomerBalanceData = async (startDate?: string, endDate?: string) => {
    ...
    setError(null);
    try {
      const response = await axiosInstance.get(`https://powerkeybackend-production.up.railway.app/api/customer-balance-summary/${selectedCompany.company_id}`, {
  ...
  useEffect(() => {
    if (selectedCompany?.company_id) {
      ...
      setError('Please select a company to view customer balance summary');
      setLoading(false);
    }
  ...
  const getTotalBalance = () => {
    return data.reduce((total, customer) => total + parseFloat(customer.total_balance_due), 0);
  };
  ...
  const handleCustomerClick = (customerId: string) => {
    navigate(`/reports/customer-balance-detail/${customerId}`);
  };
  ...
  const handleDownloadPDF = async () => {
    try {
      if (printRef.current) {
        ...
        const filename = `customer-balance-summary-${new Date().toISOString().split('T')[0]}.pdf`;
        pdf.save(filename);
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center">
          <p className="text-gray-600">Please select a company to view customer balance summary.</p>
          <button
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="relative top-4 mx-auto p-5 border w-full max-w-7xl shadow-lg rounded-md bg-white">
            <div className="flex justify-between items-center mb-4">
              <h1 className="text-2xl font-bold mb-4">Customer Balance Summary</h1>
            ...
            <div id="print-content">
              <div className="flex justify-between items-center mb-4">
                <p className="text-sm">Outstanding Customer Balances - {selectedCompany.name}</p>
                ...
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    {data.map((customer, index) => (
                      ...
                      >
                        <td className="p-2 border-b">{customer.customer_name}</td>
                        <td className="p-2 border-b">{customer.email || 'N/A'}</td>
                        <td className="p-2 border-b">{customer.phone || 'N/A'}</td>
                        <td className="p-2 border-b text-right">{formatCurrency(customer.total_balance_due)}</td>
                      </tr>
                    ))}
                    <tr>
          ...
          <div className="relative top-4 mx-auto p-5 border w-full max-w-5xl shadow-lg rounded-md bg-white">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-medium text-gray-900">
                Print Preview - Customer Balance Summary
              </h3>
              <button
            ...
            <div className="overflow-y-auto max-h-[70vh]">
              ...
              >
                ...
                <div className="flex justify-between items-start border-b pb-4 mb-6">
                  <div>
                    <h1 className="text-3xl font-bold mb-2">Customer Balance Summary</h1>
                    <h2 className="text-xl text-gray-600 mb-2">Outstanding Customer Balances</h2>
                  ...
                  <tbody>
                    {data.map((customer, index) => (
                      <tr key={customer.id}>
                        ...
                        <td className="p-2 border-b">{customer.email || 'N/A'}</td>
                        <td className="p-2 border-b">{customer.phone || 'N/A'}</td>
                        <td className="p-2 border-b text-right font-medium">{formatCurrency(customer.total_balance_due)}</td>
                      </tr>
                    ))}
                    <tr>
                      <td className="p-2 border-t-2 border-gray-800 font-bold" colSpan={3}>TOTAL OUTSTANDING BALANCE</td>
