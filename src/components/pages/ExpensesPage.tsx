...
export default function ExpensesPage() {
  ...
  const fetchExpenses = async () => {
    try {
      ...
      const response = await axiosInstance.get(`http://147.79.115.89:3000/api/getExpenses/${selectedCompany?.company_id}`);
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this expense?')) {
      try {
        await axiosInstance.delete(`http://147.79.115.89:3000/api/deleteExpense/${selectedCompany?.company_id}/${id}`);
        fetchExpenses();
      } catch (error) {
