...
export default function ExpensesPage() {
  ...
  const fetchExpenses = async () => {
    try {
      ...
      const response = await axiosInstance.get(`https://powerkey-backend-1.onrender.com/api/getExpenses/${selectedCompany?.company_id}`);
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this expense?')) {
      try {
        await axiosInstance.delete(`https://powerkey-backend-1.onrender.com/api/deleteExpense/${selectedCompany?.company_id}/${id}`);
        fetchExpenses();
      } catch (error) {
