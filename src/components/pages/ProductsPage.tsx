...
export default function ProductsPage() {
  ...
  const fetchCategories = async () => {
    try {
      const response = await axiosInstance.get(`http://147.79.115.89:3000/api/getCategories/${selectedCompany?.company_id}`);
  ...
  const fetchVendors = async () => {
    try {
      const response = await axiosInstance.get(`http://147.79.115.89:3000/api/products/${selectedCompany?.company_id}/vendors`);
  ...
  const handleProductSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingProduct) {
        ...
        await axiosInstance.post(`http://147.79.115.89:3000/api/products/${selectedCompany?.company_id}`, data, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
      }
      fetchProducts();
  ...
  const handleCategorySubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCategory) {
        await axiosInstance.put(`http://147.79.115.89:3000/api/categories/${selectedCompany?.company_id}/${editingCategory.id}`, {
          name: categoryFormData.name,
        });
      } else {
        await axiosInstance.post(`http://147.79.115.89:3000/api/createCategory/${selectedCompany?.company_id}`, {
          name: categoryFormData.name,
        });
      }
      fetchCategories();
  ...
  const handleDeleteProduct = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this product?')) {
      try {
        await axiosInstance.delete(`http://147.79.115.89:3000/api/products/${selectedCompany?.company_id}/${id}`);
        fetchProducts();
      } catch (error) {
  ...
  const handleDeleteCategory = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this category?')) {
      try {
        await axios.delete(`http://147.79.115.89:3000/api/categories/${selectedCompany?.company_id}/${id}`);
        fetchCategories();
      } catch (error) {
              ...
              <form onSubmit={handleProductSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  {productFormData.image && (
                    <div className="mt-2">
                      <img src={`http://147.79.115.89:3000${productFormData.image}`} alt="Product preview" className="h-20 w-20 object-cover rounded" />
                    </div>
                  )}
                </div>
