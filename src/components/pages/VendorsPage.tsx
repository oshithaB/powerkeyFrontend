...
export default function VendorsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingVendor) {
        await axiosInstance.put(`https://powerkey-backend-1.onrender.com/api/updateVendors/${selectedCompany?.company_id}/${editingVendor.vendor_id}`, payload);
      } else {
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this vendor?')) {
      try {
        await axiosInstance.put(`https://powerkey-backend-1.onrender.com/api/deleteVendors/${selectedCompany?.company_id}/${id}`);
        fetchVendors();
        setError(null);
      } catch (error: any) {
