...
export default function EditBill() {
    ...
    const initialFormData = {
        ...
        mark_as_paid: bill ? (bill.status === 'paid' || bill.paid_amount >= bill.total_amount) : false,
    };
            ...
            setItems(response.data.map((item: any) => {
                ...
                if (qty > 0 && unit > 0) {
                    ...
                    if (taxAmount > 0.01) {
                        ...
                        taxRate = Number(calculatedRate.toFixed(4));
                    }
                }
    ...
    const calculateTotal = () => {
        const subtotal = Number(items.reduce((sum, item) => sum + (Number(item.quantity) * Number(item.unit_price)), 0).toFixed(4));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const submitData = {
                ...
                payment_method: paymentMethodToUse ? parseInt(paymentMethodToUse as any) : 1,
            ...
            if (bill) {
                await axiosInstance.put(`http://147.79.115.89:3000/api/updateBill/${selectedCompany?.company_id}/${bill.id}`, submitData);
            } else {
                                        ...
                                        {items.map((item, index) => (
                                            ...
                                            >
                                                ...
                                                <td className="px-4 py-2 text-center border border-gray-200">
                                                    Rs. {Number(item.total_price).toFixed(4)}
                                                </td>
                                                <td className="px-4 py-2">
                                                    <button
                                                        type="button"
