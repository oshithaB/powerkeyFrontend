...
export default function InvoiceModal({ invoice, onSave }: InvoiceModalProps) {
  ...
  const fetchCustomerEstimates = async (customerId: string) => {
    ...
    try {
      const response = await axiosInstance.get(`http://147.79.115.89:3000/api/getEstimatesByCustomer/${selectedCompany?.company_id}/${customerId}`);
  ...
  const loadEstimateData = async (estimateId: string) => {
    ...
    try {
      const [estimateRes, itemsRes] = await Promise.all([
        axiosInstance.get(`http://147.79.115.89:3000/api/getEstimates/${selectedCompany?.company_id}`),
  ...
  const handleSubmit = async (e: React.FormEvent, invoiceType?: 'invoice' | 'proforma') => {
    ...
    try {
      ...
      if (invoice) {
        response = await axiosInstance.put(`http://147.79.115.89:3000/api/invoices/${selectedCompany?.company_id}/${invoice.id}`, submitData);
      } else {
        ...
        try {
          ...
          const response = await axiosInstance.post(
            `http://147.79.115.89:3000/api/createInvoice/${selectedCompany?.company_id}`,
            submitData
          );
          ...
          if (formData.estimate_id) {
            await axiosInstance.post(`http://147.79.115.89:3000/api/updateEstimateAfterInvoice/${selectedCompany?.company_id}/${formData.estimate_id}`, {
