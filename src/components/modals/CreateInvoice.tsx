...
export default function InvoiceModal({ invoice, onSave }: InvoiceModalProps) {
  ...
  const fetchCustomerEstimates = async (customerId: string) => {
    ...
    try {
      const response = await axiosInstance.get(`https://powerkey-backend-1.onrender.com/api/getEstimatesByCustomer/${selectedCompany?.company_id}/${customerId}`);
  ...
  const loadEstimateData = async (estimateId: string) => {
    ...
    try {
      const [estimateRes, itemsRes] = await Promise.all([
        axiosInstance.get(`https://powerkey-backend-1.onrender.com/api/getEstimates/${selectedCompany?.company_id}`),
        axiosInstance.get(`https://powerkey-backend-1.onrender.com/api/estimatesItems/${selectedCompany?.company_id}/${estimateId}`)
      ]);
  ...
  const handleSubmit = async (e: React.FormEvent, invoiceType?: 'invoice' | 'proforma') => {
    ...
    try {
      ...
      if (invoice) {
        response = await axiosInstance.put(`https://powerkey-backend-1.onrender.com/api/invoices/${selectedCompany?.company_id}/${invoice.id}`, submitData);
      } else {
        ...
        try {
          ...
          if (userRole !== 'admin' && submitData.status !== 'proforma') {
            const eligibilityRes = await axiosInstance.post(`https://powerkey-backend-1.onrender.com/api/checkCustomerEligibility`, {
              company_id: selectedCompany?.company_id,
          ...
          const response = await axiosInstance.post(
            `https://powerkey-backend-1.onrender.com/api/createInvoice/${selectedCompany?.company_id}`,
            submitData
          );
          ...
          if (formData.estimate_id) {
            await axiosInstance.post(`https://powerkey-backend-1.onrender.com/api/updateEstimateAfterInvoice/${selectedCompany?.company_id}/${formData.estimate_id}`, {
